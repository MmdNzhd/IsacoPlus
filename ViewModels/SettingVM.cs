using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;

namespace KaraYadak.ViewModels
{

    public class SettingVM
    {
        public string AboutUs { get; set; }
        public string Address { get; set; }
        public string ContactUs { get; set; }
        public string Email { get; set; }
        public string Facebook { get; set; }
        public string Footer { get; set; }
        public string Instagram { get; set; }
        public string linkedin { get; set; }
        public string Phone { get; set; }
        public string PhoneNumber { get; set; }
        public string SiteLogo { get; set; }
        public string SiteTitle { get; set; }
        public string Slider1 { get; set; }
        public string Slider2 { get; set; }
        public string Slider3 { get; set; }
        public string Slider1Text { get; set; }
        public string Slider2Text { get; set; }
        public string Slider3Text { get; set; }
        public string Slider4Text { get; set; }
        public string Slider5Text { get; set; }
        public string Slider1Pic { get; set; }
        public string Slider2Pic { get; set; }
        public string Slider3Pic { get; set; }
        public string Slider4Pic { get; set; }
        public string Slider5Pic { get; set; }
        public string Successful { get; set; }
        public string Telegram { get; set; }
        public string Twitter { get; set; }
        public string whatsapp { get; set; }
        public string Category1 { get; set; }
        public string Category2 { get; set; }
        public string Category3 { get; set; }
        public string Category4 { get; set; }
        public string Category5 { get; set; }
        public string Category6 { get; set; }
        public string Category7 { get; set; }
        public string Category1Title { get; set; }
        public string Category2Title { get; set; }
        public string Category3Title { get; set; }
        public string Category4Title { get; set; }
        public string Category5Title { get; set; }
        public string Category6Title { get; set; }
        public string Category7Title { get; set; }
    }

    public class SiteSettingVM
    {
        public string AboutUs { get; set; }
        public string ContactUs { get; set; }
        public string Footer { get; set; }
        public string Slider1Text { get; set; }
        public string Slider2Text { get; set; }
        public string Slider3Text { get; set; }
        public string Slider4Text { get; set; }
        public string Slider5Text { get; set; }
    }

    public class GeneralSettingVM
    {
        public string Address { get; set; }
        public string Email { get; set; }
        public string Facebook { get; set; }
        public string Instagram { get; set; }
        public string linkedin { get; set; }
        public string Phone { get; set; }
        public string PhoneNumber { get; set; }
        public string SiteTitle { get; set; }
        public string Successful { get; set; }
        public string Telegram { get; set; }
        public string Twitter { get; set; }
        public string whatsapp { get; set; }
    }
}
